...
- 배포/실행 스크립트: `scripts/start_web.sh`, `scripts/run_ingest.sh`, `scripts/run_daily_report.sh`.
- 설정 로드: daily_report 실행 시 `python-dotenv`로 `.env`를 우선 로드(find_dotenv usecwd=True, override=False) 후 settings 사용.
