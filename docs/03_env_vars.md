...
| TZ | Asia/Seoul | `app/config.py`, 시간 계산 전역 | `stats`/요약/ingest에서 KST 변환. |
| DATABASE_URL | 없음 | `app/db.py`, `app/jobs/ingest.py`, `app/jobs/daily_report.py` | Postgres 권장(JSONB, timezone 함수). 없으면 DB 세션 생성 실패. |
...
| OPENAI_API_KEY | 없음 | `app/llm_client.py`, `app/jobs/daily_report.py` | 없으면 daily_report 실행 시 RuntimeError. |
...
| SUMMARY_LANGUAGE | ko | `app/services/summary_service.py`, `app/jobs/daily_report.py` | 요약/리포트 언어. |
| MAX_THREADS_PER_DAILY_REPORT | 60 | `app/jobs/daily_report.py` | 채널별 리포트에 포함할 최대 스레드 수. |
| .env 로드 | - | `app/jobs/daily_report.py` | `python-dotenv`로 `find_dotenv(filename=\".env\", usecwd=True)` 호출 후 load(override=False). 환경변수가 우선. |
